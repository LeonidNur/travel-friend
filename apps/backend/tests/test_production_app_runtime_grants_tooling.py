...
def test_production_grants_cover_the_audited_runtime_matrix() -> None:
    ...
    assert "GRANT UPDATE (id) ON TABLE public.chats TO app_runtime;" not in script
    assert "GRANT UPDATE (id) ON TABLE public.chat_participants TO app_runtime;" not in script
    assert "GRANT INSERT (type) ON TABLE public.chats TO app_runtime;" not in script
    assert "GRANT INSERT (chat_id, user_id) ON TABLE public.chat_participants TO app_runtime;" not in script
    assert "GRANT INSERT (chat_id, sender_user_id, recipient_user_id, sequence_number, type, content_text)" not in script
    ...
    assert "GRANT INSERT (chat_id, created_by_user_id, status) ON TABLE public.trips TO app_runtime;" not in script
    assert "GRANT INSERT (trip_id, user_id) ON TABLE public.trip_participants TO app_runtime;" not in script

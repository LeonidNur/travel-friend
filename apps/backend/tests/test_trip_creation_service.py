"""Unit coverage for direct Chat Trip creation transaction branches."""
...
from travel_friend_backend.routers.trips import create_trip_for_direct_chat
...
def test_create_trip_inserts_server_derived_participants() -> None:
    ...
    trip = create_trip_for_direct_chat(
...
def test_create_trip_rejects_broken_direct_chat_membership() -> None:
    chat_id, creator_id = uuid4(), uuid4()
    connection = FakeConnection(
        [
            QueryResult({"id": chat_id}),
            QueryResult(),
            QueryResult(rows=[{"user_id": creator_id}]),
        ]
    )
...
def test_create_trip_route_delegates_to_the_transactional_use_case(monkeypatch) -> None:
    ...
    monkeypatch.setattr(trips, "create_trip_for_direct_chat", lambda *_: expected)

...
Этот документ описывает security-контур Travel Friend на уровне архитектуры. Реализована server-side проверка raw Telegram `initData`, Bearer sessions и RLS slices для profile/discover, Chat и Trip reads; block/report/audit, provider redirect и остальные capabilities ниже пока не реализованы. Он не задаёт таблицы БД, RLS, API или persistence-реализацию.
...
Для security cutover `988aeb0` → `b25e08dce158b7fe7ea6d17b40d96c3e893330fa` точный обязательный порядок, maintenance boundary и rollback limits заданы в [production security migrations runbook](production-security-migrations-runbook.md). Grants — самостоятельный, последующий шаг после migrations; этот work item их намеренно не предоставляет. Production Render `DATABASE_URL` уже использует app_runtime Session Pooler DSN и не меняется в этом cutover; Auto-Deploy должен быть выключен. Verification script только читает catalogs: он показывает effective role attributes, owner каждой current application table и memberships. `NOINHERIT` не удаляет существующие memberships и не запрещает возможный `SET ROLE`, поэтому любой результат в последнем query — finding для отдельного approved remediation, не повод менять memberships этим provisioning script.

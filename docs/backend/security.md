...
Gate fail closed: подтверждает `current_user`/`session_user`, непривилегированные attributes роли, существование и `EXECUTE` нужных security capabilities, RLS и точные policy surfaces для `profiles`/`travel_intents`/`user_activity_states`/`user_sessions`, Chat и Trip slices. Для `chats`, `chat_participants`, `messages`, `trips`, `trip_participants` и `trip_stops` он также проверяет минимальную column surface и отсутствие direct mutations. Он проверяет transaction-local `app.user_id`, отсутствие утечки context в следующую transaction и fail-closed RLS без context. Отрицательные mutation probes выполняются с `WHERE false` и всегда завершаются rollback, поэтому не оставляют изменения. Только результат `passed` разрешает следующий ручной шаг переключения DSN; сам script Render не изменяет.

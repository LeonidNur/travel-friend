...
Gate fail closed: подтверждает `current_user`/`session_user`, непривилегированные attributes роли, существование и `EXECUTE` нужных security capabilities, RLS и точные policy surfaces для `profiles`/`travel_intents`/`user_activity_states`/`user_sessions` и Chat slice. Для `chats`, `chat_participants` и `messages` он также проверяет отсутствие direct insert/delete и других mutation grants, а также минимальную column surface. Он проверяет transaction-local `app.user_id`, отсутствие утечки context в следующую transaction, fail-closed RLS без context, безопасный boolean-only runtime read и запрет `DELETE`. Он не создаёт fixture-данные: отрицательный `DELETE ... WHERE false` всегда завершается rollback и не может оставить изменения. Только результат `passed` разрешает следующий ручной шаг переключения DSN; сам script Render не изменяет.
...
Временный **Trip compatibility bridge** остаётся только до отдельного Trip RLS slice: `UPDATE(id)` на `chats` и `chat_participants` нужен текущему Trip creation flow для `SELECT ... FOR UPDATE` / `FOR SHARE`. Соответствующие Chat RLS lock policies ограничены active membership и имеют `WITH CHECK (false)`, поэтому runtime не может выполнить реальную mutation. Bridge не предоставляет ни других UPDATE columns, ни INSERT/DELETE; следующий Trip RLS slice обязан удалить оба grants и lock policies.

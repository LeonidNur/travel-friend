...
Для authenticated business request FastAPI после успешной Bearer authentication является единственным источником `app.user_id`: dependency открывает отдельный explicit outer transaction и выполняет параметризованный `set_config('app.user_id', <principal.user_id>, true)` до router/service SQL. Флаг `true` делает контекст transaction-local; при commit или rollback он больше не представляет identity запроса. Telegram login/first-login bootstrap остаётся следующим отдельным slice: runtime role пока сохраняет прямые `INSERT` для создания session и `UPDATE` для logout. Для условия текущего logout PostgreSQL также требует column-level `SELECT(id)`; table-level `SELECT` и чтение `token_hash` отсутствуют, а Bearer resolution идёт только через capability.
